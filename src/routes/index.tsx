  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-8">
      <TopBar level={level} into={into} need={need} xp={progress.xp} streak={progress.streak}
        apiKey={apiKey} onApiKeyChange={setApiKey} onClearApiKey={clearApiKey}
      ...
      <section className="glass-card mt-6 p-6 md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex gap-2">
            <label className="chip cursor-pointer hover:bg-card">
              📎 Upload .txt
        ...
        <textarea
          ...
          className="mt-4 h-28 w-full resize-none rounded-2xl border border-border bg-card/70 p-4 text-sm outline-none focus:border-primary"
        />
      ...
      <section className="mt-6">
        {tab === "cards" && <Flashcards items={lesson.vocabulary} onXP={(n) => addXP(n, "learn5")} apiKey={apiKey} />}
        {tab === "match" && (
      ...
      {levelUp && (
        <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center">
          <div className="animate-pop-in rounded-3xl bg-card/90 px-8 py-6 text-center shadow-2xl backdrop-blur">
...
/* ---------------- Top bar ---------------- */
function TopBar({ level, into, need, xp, streak, apiKey, onApiKeyChange, onClearApiKey, theme, onToggleTheme }: {
  level: number; into: number; need: number; xp: number; streak: number;
  apiKey: string; onApiKeyChange: (key: string) => void; onClearApiKey: () => void;
  ...
  return (
    <header className="glass-card flex flex-col gap-3 p-4 md:flex-row md:items-center md:gap-6 md:p-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <AuthPanel />
        <SettingsPanel apiKey={apiKey} onApiKeyChange={onApiKeyChange} onClear={onClearApiKey} />
      </div>
    </header>
  );
}
...
/* ---------------- Flashcards ---------------- */
function Flashcards({ items, onXP, apiKey }: { items: VocabItem[]; onXP: (n: number) => void; apiKey?: string }) {
  ...
  async function generateImage(v: VocabItem) {
    ...
    try {
      ...
      if (apiKey) imgHeaders["X-Client-API-Key"] = apiKey;
    ...
    <div className="grid gap-6 md:grid-cols-[1fr_260px]">
      <div>
        ...
        <div
          ...
          className="relative min-h-72 cursor-pointer rounded-3xl bg-card p-8 shadow-[var(--shadow-pop)] transition hover:-translate-y-0.5"
        >
          {!flipped ? (
              ...
              <li key={v.word} className="flex items-center gap-1">
                <button
                  ...
                  className={`flex flex-1 items-center gap-2 rounded-xl px-3 py-2 text-left text-sm transition ${
                    idx === i ? "bg-primary/20 font-bold" : "hover:bg-card"
                  }`}
                >
                  {img ? (
    ...
    <div className="glass-card p-6">
      ...
      <div className="space-y-3">
        {data.dialogue.map((d, idx) => (
          <div key={idx} className="rounded-2xl bg-card p-4">
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[320px_1fr] md:items-center">
      <div className="relative mx-auto h-72 w-72">
        <div
          className="h-full w-full rounded-full border-8 border-card shadow-[var(--shadow-pop)] transition-transform duration-[3500ms] ease-out"
          style={{
        ...
        <div className="absolute left-1/2 top-1/2 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-card text-2xl font-black shadow-lg">🎡</div>
          ...
          return (
            <div
              ...
              className={`flex flex-col items-center gap-1 rounded-2xl p-3 text-center transition ${
                owned ? "bg-lemon shadow-md" : "bg-card/50 opacity-50 grayscale"
              }`}
            >
