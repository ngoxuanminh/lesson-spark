...
export const Route = createFileRoute("/api/lesson")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const apiKey = request.headers.get("X-Client-API-Key") || process.env.COACHIO_API_KEY;
        if (!apiKey) {
          return new Response(
            JSON.stringify({ error: "API key not configured. Open Settings (⚙️) to add your Coachio API key." }),
